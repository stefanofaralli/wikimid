...
public class BabelNetMapperEN extends Mapper<BabelSynset> {
    ...
    BabelNetCandidatesEN candidatesScreenName;
    ...
    private BabelNetMapperEN() {
        ...
        candidatesScreenName = new BabelNetCandidatesEN(CandidateMethod.SCREENNAME_DECOMPOSITION);
        bc = new BabelNetContextEN();
        mapping = new HashMap<TwitterUser, DoubleCounter<BabelSynset>>();
    }

    public static BabelNetMapperEN getInstance(String source) {
        if (instances == null) {
            instances = new HashMap<String, BabelNetMapperEN>();
        }
        if (instances.containsKey(source)) {
            return instances.get(source);
        } else {
            instances.put(source, new BabelNetMapperEN());
            return instances.get(source);
        }
    }
 ...
 public DoubleCounter<BabelSynset> disambiguate2(TwitterUser2 tu, List<BabelSynset> candidates) {
    ...
    public void onFlyMap(TwitterPopulation tp, String outputfile) {
        ...
        try {
            ...
            Logger.getLogger(BabelNetMapperEN.class.getName()).log(Level.SEVERE, null, ex);
        }
